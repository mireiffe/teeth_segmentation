...
class Snake():
    ...
    def snake(self):
        ...
        lbl_er = label(oma, background=0)
        for l in np.unique(lbl_er)[1:]:
            _r = np.where(lbl_er == l, 1., 0)
            if _r.sum() < 3:
                oma = np.where(lbl_er == l, 0, oma)
        ...
        while True:
            ...
            if k in [1, 2] or k % 1 == 0:
                ...
                plt.imshow(self.bar_er[85:152, 140:166], 'gray')
                # plt.imshow((Fb + Fc).sum(axis=0)[85:155, 110:163], vmin=-0.3, cmap=mts.colorMapAlpha(plt, cmap='rainbow'))
                plt.imshow(oma[85:152, 140:166], cmap=mts.colorMapAlpha(plt, cmap='gnuplot'), vmax=1.1)
